...
class Aggregator(object):
    ...
    def _mix_neighbor_vectors(self, neighbor_vectors, neighbor_relations, user_embeddings):
        ...
        if not avg:
            # [batch_size, 1, 1, dim]
            dynamic_batch_size = tf.shape(user_embeddings)[0]
            
            expected_num_elements = dynamic_batch_size * self.dim  # 预期的元素总数
            #user_embeddings = tf.reshape(user_embeddings, [self.batch_size, 1, 1, self.dim])
            user_embeddings = tf.reshape(user_embeddings, [dynamic_batch_size, 1, 1, self.dim])
            # 确认 user_embeddings 的元素总数是否与预期相符
            actual_num_elements = tf.size(user_embeddings)
            assert_op = tf.Assert(tf.equal(expected_num_elements, actual_num_elements), [expected_num_elements, actual_num_elements])
            with tf.control_dependencies([assert_op]):
            # 如果没有错误，继续执行重塑操作
                user_embeddings_reshaped = tf.reshape(user_embeddings, [dynamic_batch_size, 1, 1, self.dim])
