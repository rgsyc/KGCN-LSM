...
def train(args, data, show_loss, show_topk,user_features):
    n_user, n_item, n_entity, n_relation = data[0], data[1], data[2], data[3]
    train_data, eval_data, test_data = data[4], data[5], data[6]
    adj_entity, adj_relation = data[7], data[8]

    model = KGCN(args, n_user, n_entity, n_relation, adj_entity, adj_relation,user_features)
    ...
    with tf.Session() as sess:
        ...
        for step in range(args.n_epochs):
            ...
            while start + args.batch_size <= train_data.shape[0]:
                ...
                feed_dict = {
                    model.user_indices: batch_data[:, 0],
                    model.item_indices: batch_data[:, 1],
                    model.labels: batch_data[:, 2],
                }
