...
def load_user_features():
    user_features_path = './user_features.csv'  # 确保路径正确
    if os.path.exists(user_features_path):
        return np.loadtxt(user_features_path, delimiter=",")
    else:
        raise FileNotFoundError('User features file not found.')
...
def load_data(args):
    ...
    user_features = load_user_features()  # 加载用户特征
    return n_user, n_item, n_entity, n_relation, train_data, eval_data, test_data, adj_entity, adj_relation, user_features
