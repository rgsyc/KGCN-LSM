...
#data = load_data(args,user_features)
#train(args, data, show_loss, show_topk,user_features)


# 确保从 load_data 函数接收所有返回的数据，包括 user_features
n_user, n_item, n_entity, n_relation, train_data, eval_data, test_data, adj_entity, adj_relation, user_features = load_data(args)


# 将 data 和 user_features 作为参数传递给 train 函数
data = (n_user, n_item, n_entity, n_relation, train_data, eval_data, test_data, adj_entity, adj_relation)
train(args, data, show_loss, show_topk, user_features)
