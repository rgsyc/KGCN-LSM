...
class KGCN(object):
    def __init__(self, args, n_user, n_entity, n_relation, adj_entity, adj_relation,user_features):
        # self.user_features = user_features  # 保存用户特征
        # self._parse_args(args, adj_entity, adj_relation)
        # self._build_inputs()
        # self._build_model(n_user, n_entity, n_relation)
        # self._build_train()
        #self.user_features_initial = np.loadtxt(user_feature_path, delimiter=",")
        self.user_features = tf.Variable(initial_value=user_features, dtype=tf.float32, trainable=True, name='user_features')
        self._parse_args(args, adj_entity, adj_relation)
        self._build_inputs()
        self._build_model(n_user, n_entity, n_relation)
        self._build_train()
    ...
    def _build_model(self, n_user, n_entity, n_relation):
        ...
        #self.user_emb_matrix = tf.Variable(
            #initial_value=self.user_features, dtype=tf.float32, trainable=True, name='user_emb_matrix')
        ...
        self.kgcn_user_emb_matrix = tf.Variable(
            initial_value=tf.random.normal([n_user, self.dim]), dtype=tf.float32, trainable=True, name='kgcn_user_emb_matrix')
        self.kgcn_user_embeddings = tf.nn.embedding_lookup(self.kgcn_user_emb_matrix, self.user_indices)
        self.lstm_user_embeddings = tf.nn.embedding_lookup(self.user_features, self.user_indices)
        self.user_embeddings = self.kgcn_user_embeddings * 0.9 + self.lstm_user_embeddings * 0.1
        # [batch_size, dim]
...
#         self.optimizer = tf.train.AdamOptimizer(self.lr).minimize(self.loss)
    def _build_train(self):
        ...
        # 更新：计算 L2 正则化损失时包括所有需要的可训练变量
        self.l2_loss = tf.nn.l2_loss(self.kgcn_user_emb_matrix) + tf.nn.l2_loss(self.entity_emb_matrix) + tf.nn.l2_loss(self.relation_emb_matrix)
    
        # 如果您的其他aggregator有可训练的参数，也需要包括它们的L2损失
        for aggregator in self.aggregators:
